		...
		while (true) {
			...
			try {
				...
				if (mgmt.hasBeat()) {
					...
					JobManager.getInstance().addToChannelMap(msg.req.getJobBid().getJobId(), msg.channel);
					JobManager.getInstance().processRequest(mgmt.getJobBid());
				} else if (mgmt.hasJobPropose()) {
					JobManager.getInstance().addToChannelMap(msg.req.getJobPropose().getJobId(), msg.channel);
					JobManager.getInstance().processRequest(mgmt.getJobPropose());
				} else
