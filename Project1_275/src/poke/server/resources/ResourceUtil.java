...
public class ResourceUtil {
	 ...
	 * @param status
	 * @param statusMsg
	 * @param photoHeaderResponse
	 * @return
	 */
	public static Header buildHeaderFrom(Header reqHeader, PokeStatus status, String statusMsg, ResponseFlag photoHeaderResponse) {
		return buildHeader(reqHeader.getRoutingId(), status, statusMsg, reqHeader.getOriginator(), reqHeader.getTag(), photoHeaderResponse);
	}
	 ...
	 * @return
	 */
	public static Header buildHeader(Routing path, PokeStatus status, String msg, int fromNode, String tag, ResponseFlag photoHeaderResponse) {
		Header.Builder bldr = Header.newBuilder();
		bldr.setOriginator(fromNode);
		bldr.setRoutingId(path);
		bldr.setTag(tag);
		bldr.setReplyCode(status);
		bldr.getPhotoHeaderBuilder().setResponseFlag(photoHeaderResponse);
	 ...
	 * @return
	 */
	public static Header buildHeader(Routing path, PokeStatus status, String msg, int fromNode, String tag, int toNode) {
		Header.Builder bldr = Header.newBuilder();
		bldr.setOriginator(fromNode);
		bldr.setRoutingId(path);
		bldr.setTag(tag);
