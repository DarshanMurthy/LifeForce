	...
	public class CloseHeartListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if (outgoingHB.containsValue(heart)) {
				...
				// Update the leaderNode to null when leader
				// leaves the network
				// added to handle leader fail strategy
				int leaderNode = ElectionManager.getInstance().whoIsTheLeader();
				if (heart.getNodeId() == leaderNode) {
					ElectionManager.getInstance().concludeWith(true, null);
				}	
